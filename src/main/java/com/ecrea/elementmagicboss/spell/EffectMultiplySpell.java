...
import com.ecrea.elementmagicboss.effect.EffectMultiplyEffect;
...
/**
 * エフェクト倍化 (Effect Multiply)
 * ターゲットにエフェクト倍化デバフを付与する。
 * デバフ中に受けるデバフの持続時間が multiplier 倍になる。
 * School: Evocation / CastType: LONG / Cooldown: 20s / CastTime: 1s
 *
 * Lv1: x2.0 / 10s
 * Lv2: x2.5 / 14s
 * LvN: x(2.0 + 0.5*(N-1)) / (10 + 4*(N-1))s
 */
public class EffectMultiplySpell extends AbstractSpell {

    private final ResourceLocation spellId =
            new ResourceLocation(ElementMagicBossMod.MOD_ID, "effect_multiply");
    ...
    public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
        float multiplier = EffectMultiplyEffect.getMultiplier(spellLevel - 1);
        return List.of(
                Component.translatable("ui.elementmagicboss.effect_multiply_rate",
        ...
        if (playerMagicData.getAdditionalCastData() instanceof TargetEntityCastData targetData
            ...
            if (target != null) {
                target.addEffect(new MobEffectInstance(
                        ModMobEffects.EFFECT_MULTIPLY.get(),
