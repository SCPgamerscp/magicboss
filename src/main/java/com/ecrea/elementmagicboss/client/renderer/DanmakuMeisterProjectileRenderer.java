...
import com.ecrea.elementmagicboss.entity.projectile.DanmakuMeisterProjectile;
...
public class DanmakuMeisterProjectileRenderer extends EntityRenderer<DanmakuMeisterProjectile> {
    ...
    public DanmakuMeisterProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.shadowRadius = 0.0f;
    }

    @Override
    public void render(DanmakuMeisterProjectile entity, float yaw, float partialTicks,
        ...
        renderSprite(poseStack, bufferSource, getTextureLocation(entity), entity.getBulletColor().rgb());
        poseStack.popPose();
        super.render(entity, yaw, partialTicks, poseStack, bufferSource, light);
    }

    private static void renderSprite(PoseStack poseStack, MultiBufferSource bufferSource, ResourceLocation texture, int rgb) {
        Pose pose = poseStack.last();
        Matrix4f poseMatrix = pose.pose();
        Matrix3f normalMatrix = pose.normal();
        VertexConsumer consumer = bufferSource.getBuffer(RenderHelper.CustomerRenderType.magic(texture));
        vertex(poseMatrix, normalMatrix, consumer, -0.5f, -0.5f, 0.0f, 0.0f, 1.0f, rgb);
        vertex(poseMatrix, normalMatrix, consumer, 0.5f, -0.5f, 0.0f, 1.0f, 1.0f, rgb);
        vertex(poseMatrix, normalMatrix, consumer, 0.5f, 0.5f, 0.0f, 1.0f, 0.0f, rgb);
        vertex(poseMatrix, normalMatrix, consumer, -0.5f, 0.5f, 0.0f, 0.0f, 0.0f, rgb);
    }
    ...
    @Override
    public ResourceLocation getTextureLocation(DanmakuMeisterProjectile entity) {
        ScarletMeisterProjectile.BulletType type = entity.getBulletType();
        return switch (type) {
