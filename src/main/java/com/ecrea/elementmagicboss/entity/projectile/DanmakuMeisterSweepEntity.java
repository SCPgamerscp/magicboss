...
public class DanmakuMeisterSweepEntity extends Entity {
    ...
    public DanmakuMeisterSweepEntity(EntityType<? extends DanmakuMeisterSweepEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
        this.setNoGravity(true);
    }

    public DanmakuMeisterSweepEntity(Level level, LivingEntity owner, float projectileDamage) {
        this(ModEntities.DANMAKU_MEISTER_SWEEP.get(), level);
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            if (tickCount >= DanmakuMeisterSpell.DURATION_TICKS) discard();
            return;
        }
        int sweepTick = tickCount - 1;
        if (sweepTick >= 0 && sweepTick < DanmakuMeisterSpell.DURATION_TICKS) {
            spawnSweep(sweepTick);
        }
        if (tickCount >= DanmakuMeisterSpell.DURATION_TICKS) {
            discard();
        }
    }
    ...
    private void spawnSweep(int sweepTick) {
        ...
        Vec3 origin = owner != null ? owner.position().add(0.0D, owner.getEyeHeight() - 0.15D, 0.0D) : position();
        double baseAngle = INITIAL_ANGLE + (360.0D / DanmakuMeisterSpell.DURATION_TICKS) * sweepTick;

        for (int i = 0; i < DanmakuMeisterSpell.SWEEP_COUNT; i++) {
            ...
            DanmakuMeisterProjectile.BulletColor color = DanmakuMeisterProjectile.BulletColor.byId(sweepTick + i);

            spawnProjectile(origin, damageOwner, ScarletMeisterProjectile.BulletType.BUBBLE, color,
                    shotDirection.scale(speed), lifetime);
            double mid = 0.6D + 0.3D * random.nextDouble();
            spawnProjectile(origin, damageOwner, ScarletMeisterProjectile.BulletType.MENTOS, color.next(),
                    shotDirection.scale(speed * mid), (int) (lifetime / mid * 0.8D));
            double low = 0.3D + 0.3D * random.nextDouble();
            spawnProjectile(origin, damageOwner, ScarletMeisterProjectile.BulletType.BALL, color.next().next(),
    ...
    private void spawnProjectile(Vec3 origin, Entity owner, ScarletMeisterProjectile.BulletType bulletType,
                                 DanmakuMeisterProjectile.BulletColor color, Vec3 velocity, int lifetime) {
        DanmakuMeisterProjectile projectile = new DanmakuMeisterProjectile(level(), owner);
        projectile.setPos(origin.x, origin.y, origin.z);
        projectile.setDamage(projectileDamage);
        projectile.setDamageSpellId(ModSpells.DANMAKU_MEISTER.getId().toString());
        projectile.setBulletType(bulletType);
        projectile.setBulletColor(color);
        projectile.setMaxLifetimeTicksCustom(lifetime);
        projectile.setDeltaMovement(velocity);
        projectile.addTag(DanmakuMeisterSpell.DANMAKU_MEISTER_PROJECTILE_TAG);
